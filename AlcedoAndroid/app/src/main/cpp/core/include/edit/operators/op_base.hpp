// AlcedoAndroid - Operator base: stage/param enums, OperatorParams, CRTP base.
// Adapted from the desktop project: OpenColorIO/OpenCV types removed, RAW color
// context + lens-calib runtime pulled from the Android metadata headers, and the
// ~30 adjustment parameters retained so the operator set is feature-complete.
...
// The 7 canonical pipeline stages (+ metadata sentinels).
...
// Operator type enumeration (~27 types).
...
enum class ColorTempMode : int { AS_SHOT = 0, CUSTOM = 1 };
enum class RawDecodeInputSpace : int { AP0 = 0, CAMERA = 1 };

// ACES 2.0 / OpenDRT output-transform parameters (host side). Mirrors the
// desktop ColorUtils::TO_OUTPUT_Params but kept self-contained.
struct ToOutputParams {
  bool   use_open_drt_      = true;   // true: OpenDRT, false: ACES 2.0 ODT
  float  display_white_l_   = 1.0f;
  float  display_black_l_   = 0.0f;
  float  surround_gamma_    = 1.0f;
  float  grading_midpoint_  = 0.18f;
  float  grading_contrast_  = 1.0f;
  float  rendering_gamma_   = 1.0f;
  bool   apply_look_        = false;
  float  look_strength_     = 1.0f;
};

// The full set of adjustment parameters shared across all operators in a
// pipeline. Each operator reads/writes the slice it owns.
struct OperatorParams {
  static constexpr int kDetailMaxGaussianTapCount = 64;

  // ---- Tone mapping (shadows/highlights/clarity local tone) ----
  ...
  // GPU-only shared shadows/highlights tone curve.
  ...
  // Shadows/highlights local tone (Vulkan).
  ...
  // Render frame role (used by Vulkan local H/S).
  ...
  // Saturation (global OKLCh chroma scale)
  ...
  // Dynamic RAW color temperature / white balance.
  ...
  std::uint64_t       lens_calib_cache_key_          = 0;
  ...
  std::uint64_t color_temp_cache_key_ = 0;
  ...
  ToOutputParams   to_output_params_  = {};

  // Curve adjustment
  bool                       curve_enabled_ = true;
  std::vector<CurvePoint>    curve_ctrl_pts_;
  ...
  // Populate raw metadata from a runtime color context.
  void PopulateRawMetadata(const RawRuntimeColorContext& ctx) {
    ...
    for (int i = 0; i < 3; ++i) raw_as_shot_neutral_[i] = ctx.as_shot_neutral_[i];
...
class IOperatorBase {
 public:
  ...
  virtual auto GetCanonicalName() const -> std::string              = 0;
...
// CRTP base providing the static-metadata accessors.
template <typename Derived>
class OperatorBase : public IOperatorBase {
 public:
  auto GetCanonicalName() const -> std::string override {
    return std::string(Derived::canonical_name_);
  }
  auto GetScriptName() const -> std::string override {
    return std::string(Derived::script_name_);
  }
...
// Ordered container of operators executed by a stage.
struct OpStream {
