// Ported from AlcedoStudio desktop: utils/queue/queue.hpp
// SPDX-License-Identifier: GPL-3.0-only
//
// Thread-safe task queues used by the threadpool and pipeline scheduler:
//   - ConcurrentBlockingQueue<T>  : blocking MPMC over std::queue + mutex/cv
//   - LockFreeMPMCQueue<T>        : bounded lock-free MPMC ring buffer
//   - BlockingMPMCQueue<T>        : blocking wrapper over LockFreeMPMCQueue
...
/**
 * @brief A thread-safe non-blocking ring buffer (Vyukov MPMC).
 */
template <typename T>
