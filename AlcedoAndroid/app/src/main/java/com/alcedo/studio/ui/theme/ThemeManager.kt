...
object ThemeManager {
    ...
    private val _themeVariant = MutableStateFlow(AlcedoThemeVariant.GRAPHITE)
    ...
    fun initialize(context: Context) {
        ...
        val savedVariant = prefs?.getString(KEY_THEME_VARIANT, AlcedoThemeVariant.GRAPHITE.name)
            ?: AlcedoThemeVariant.GRAPHITE.name
    ...
    fun getAlcedoColorScheme(variant: AlcedoThemeVariant, darkTheme: Boolean): AlcedoColorScheme {
        return when (variant) {
            ...
            AlcedoThemeVariant.DYNAMIC -> if (darkTheme) GraphiteDarkColors else GraphiteLightColors
        }
    }
}
