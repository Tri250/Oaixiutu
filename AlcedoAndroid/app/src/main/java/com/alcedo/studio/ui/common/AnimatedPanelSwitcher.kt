 ...
 * relative to the current one, with a short fade. Uses [AnimatedContent] keyed
 * by [key].
 *
...
fun <T> AnimatedPanelSwitcher(
    ...
    val enter = if (direction >= 0) {
        slideInHorizontally(tween(DesignTokens.motionPanel)) { w -> w } + fadeIn(tween(DesignTokens.motionPanel))
    } else {
        slideInHorizontally(tween(DesignTokens.motionPanel)) { w -> -w } + fadeIn(tween(DesignTokens.motionPanel))
    }
    val exit = if (direction >= 0) {
        slideOutHorizontally(tween(DesignTokens.motionPanel)) { w -> -w } + fadeOut(tween(DesignTokens.motionPanel))
    } else {
        slideOutHorizontally(tween(DesignTokens.motionPanel)) { w -> w } + fadeOut(tween(DesignTokens.motionPanel))
    }
    AnimatedContent(
